namespace HE_THONG_BAN_XE.ControlHeThong
{
    partial class KhachHangControl
    {
        /// <summary> 
        /// Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary> 
        /// Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Component Designer generated code

        /// <summary> 
        /// Required method for Designer support - do not modify 
        /// the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            label1 = new Label();
            groupBox1 = new GroupBox();
            radioButton_nu_khachhang = new RadioButton();
            radioButton_nam_khachhang = new RadioButton();
            dateTimePicker_ngaysinh_khachhang = new DateTimePicker();
            textBox_timkiem_khachhang = new TextBox();
            textBox_diachi_khachhang = new TextBox();
            textBox_email_khachhang = new TextBox();
            textBox_sdt_khachhang = new TextBox();
            textBox_tenKH_khachhang = new TextBox();
            textBox_maKh_khachhang = new TextBox();
            label9 = new Label();
            label8 = new Label();
            label7 = new Label();
            label6 = new Label();
            label5 = new Label();
            label4 = new Label();
            label3 = new Label();
            label2 = new Label();
            groupBox2 = new GroupBox();
            button_tatca_khachhang = new Button();
            button_timkiem_khachhang = new Button();
            button_xoa_khachhang = new Button();
            button_sua_khachhang = new Button();
            button_them_khachhang = new Button();
            dataGridView_khachhang = new DataGridView();
            groupBox1.SuspendLayout();
            groupBox2.SuspendLayout();
            ((System.ComponentModel.ISupportInitialize)dataGridView_khachhang).BeginInit();
            SuspendLayout();
            // 
            // label1
            // 
            label1.AutoSize = true;
            label1.Font = new Font("Times New Roman", 13.8F, FontStyle.Bold, GraphicsUnit.Point, 0);
            label1.Location = new Point(13, 11);
            label1.Name = "label1";
            label1.Size = new Size(235, 25);
            label1.TabIndex = 0;
            label1.Text = "Quản Lý Khách Hàng";
            label1.Click += label1_Click;
            // 
            // groupBox1
            // 
            groupBox1.BackColor = Color.SkyBlue;
            groupBox1.Controls.Add(radioButton_nu_khachhang);
            groupBox1.Controls.Add(radioButton_nam_khachhang);
            groupBox1.Controls.Add(dateTimePicker_ngaysinh_khachhang);
            groupBox1.Controls.Add(textBox_timkiem_khachhang);
            groupBox1.Controls.Add(textBox_diachi_khachhang);
            groupBox1.Controls.Add(textBox_email_khachhang);
            groupBox1.Controls.Add(textBox_sdt_khachhang);
            groupBox1.Controls.Add(textBox_tenKH_khachhang);
            groupBox1.Controls.Add(textBox_maKh_khachhang);
            groupBox1.Controls.Add(label9);
            groupBox1.Controls.Add(label8);
            groupBox1.Controls.Add(label7);
            groupBox1.Controls.Add(label6);
            groupBox1.Controls.Add(label5);
            groupBox1.Controls.Add(label4);
            groupBox1.Controls.Add(label3);
            groupBox1.Controls.Add(label2);
            groupBox1.Font = new Font("Times New Roman", 12F, FontStyle.Regular, GraphicsUnit.Point, 0);
            groupBox1.Location = new Point(13, 52);
            groupBox1.Name = "groupBox1";
            groupBox1.Size = new Size(692, 328);
            groupBox1.TabIndex = 1;
            groupBox1.TabStop = false;
            groupBox1.Text = "Thông Tin Khách Hàng";
            // 
            // radioButton_nu_khachhang
            // 
            radioButton_nu_khachhang.AutoSize = true;
            radioButton_nu_khachhang.Location = new Point(412, 111);
            radioButton_nu_khachhang.Name = "radioButton_nu_khachhang";
            radioButton_nu_khachhang.Size = new Size(56, 26);
            radioButton_nu_khachhang.TabIndex = 16;
            radioButton_nu_khachhang.TabStop = true;
            radioButton_nu_khachhang.Text = "Nữ";
            radioButton_nu_khachhang.UseVisualStyleBackColor = true;
            // 
            // radioButton_nam_khachhang
            // 
            radioButton_nam_khachhang.AutoSize = true;
            radioButton_nam_khachhang.Location = new Point(257, 111);
            radioButton_nam_khachhang.Name = "radioButton_nam_khachhang";
            radioButton_nam_khachhang.Size = new Size(68, 26);
            radioButton_nam_khachhang.TabIndex = 15;
            radioButton_nam_khachhang.TabStop = true;
            radioButton_nam_khachhang.Text = "Nam";
            radioButton_nam_khachhang.UseVisualStyleBackColor = true;
            // 
            // dateTimePicker_ngaysinh_khachhang
            // 
            dateTimePicker_ngaysinh_khachhang.Location = new Point(173, 140);
            dateTimePicker_ngaysinh_khachhang.Name = "dateTimePicker_ngaysinh_khachhang";
            dateTimePicker_ngaysinh_khachhang.Size = new Size(498, 30);
            dateTimePicker_ngaysinh_khachhang.TabIndex = 14;
            // 
            // textBox_timkiem_khachhang
            // 
            textBox_timkiem_khachhang.Location = new Point(173, 287);
            textBox_timkiem_khachhang.Name = "textBox_timkiem_khachhang";
            textBox_timkiem_khachhang.Size = new Size(498, 30);
            textBox_timkiem_khachhang.TabIndex = 13;
            // 
            // textBox_diachi_khachhang
            // 
            textBox_diachi_khachhang.Location = new Point(173, 251);
            textBox_diachi_khachhang.Name = "textBox_diachi_khachhang";
            textBox_diachi_khachhang.Size = new Size(498, 30);
            textBox_diachi_khachhang.TabIndex = 12;
            // 
            // textBox_email_khachhang
            // 
            textBox_email_khachhang.Location = new Point(173, 215);
            textBox_email_khachhang.Name = "textBox_email_khachhang";
            textBox_email_khachhang.Size = new Size(498, 30);
            textBox_email_khachhang.TabIndex = 11;
            // 
            // textBox_sdt_khachhang
            // 
            textBox_sdt_khachhang.Location = new Point(173, 179);
            textBox_sdt_khachhang.Name = "textBox_sdt_khachhang";
            textBox_sdt_khachhang.Size = new Size(498, 30);
            textBox_sdt_khachhang.TabIndex = 10;
            // 
            // textBox_tenKH_khachhang
            // 
            textBox_tenKH_khachhang.Location = new Point(173, 71);
            textBox_tenKH_khachhang.Name = "textBox_tenKH_khachhang";
            textBox_tenKH_khachhang.Size = new Size(498, 30);
            textBox_tenKH_khachhang.TabIndex = 9;
            // 
            // textBox_maKh_khachhang
            // 
            textBox_maKh_khachhang.Location = new Point(173, 35);
            textBox_maKh_khachhang.Name = "textBox_maKh_khachhang";
            textBox_maKh_khachhang.Size = new Size(498, 30);
            textBox_maKh_khachhang.TabIndex = 8;
            // 
            // label9
            // 
            label9.AutoSize = true;
            label9.Font = new Font("Times New Roman", 10.8F);
            label9.Location = new Point(79, 291);
            label9.Name = "label9";
            label9.Size = new Size(88, 20);
            label9.TabIndex = 7;
            label9.Text = "Tìm Kiếm:";
            label9.Click += label9_Click;
            // 
            // label8
            // 
            label8.AutoSize = true;
            label8.Font = new Font("Times New Roman", 10.8F);
            label8.Location = new Point(99, 255);
            label8.Name = "label8";
            label8.Size = new Size(68, 20);
            label8.TabIndex = 6;
            label8.Text = "Địa Chỉ:";
            // 
            // label7
            // 
            label7.AutoSize = true;
            label7.Font = new Font("Times New Roman", 10.8F);
            label7.Location = new Point(113, 219);
            label7.Name = "label7";
            label7.Size = new Size(54, 20);
            label7.TabIndex = 5;
            label7.Text = "Email:";
            // 
            // label6
            // 
            label6.AutoSize = true;
            label6.Font = new Font("Times New Roman", 10.8F);
            label6.Location = new Point(49, 183);
            label6.Name = "label6";
            label6.Size = new Size(118, 20);
            label6.TabIndex = 4;
            label6.Text = "Số Điện Thoại:";
            // 
            // label5
            // 
            label5.AutoSize = true;
            label5.Font = new Font("Times New Roman", 10.8F);
            label5.Location = new Point(79, 147);
            label5.Name = "label5";
            label5.Size = new Size(88, 20);
            label5.TabIndex = 3;
            label5.Text = "Ngày Sinh:";
            // 
            // label4
            // 
            label4.AutoSize = true;
            label4.Font = new Font("Times New Roman", 10.8F);
            label4.Location = new Point(83, 111);
            label4.Name = "label4";
            label4.Size = new Size(84, 20);
            label4.TabIndex = 2;
            label4.Text = "Giới Tính:";
            // 
            // label3
            // 
            label3.AutoSize = true;
            label3.Font = new Font("Times New Roman", 10.8F);
            label3.Location = new Point(30, 75);
            label3.Name = "label3";
            label3.Size = new Size(137, 20);
            label3.TabIndex = 1;
            label3.Text = "Tên Khách Hàng:";
            // 
            // label2
            // 
            label2.AutoSize = true;
            label2.Font = new Font("Times New Roman", 10.8F);
            label2.Location = new Point(35, 39);
            label2.Name = "label2";
            label2.Size = new Size(132, 20);
            label2.TabIndex = 0;
            label2.Text = "Mã Khách Hàng:";
            label2.Click += label2_Click;
            // 
            // groupBox2
            // 
            groupBox2.Controls.Add(button_tatca_khachhang);
            groupBox2.Controls.Add(button_timkiem_khachhang);
            groupBox2.Controls.Add(button_xoa_khachhang);
            groupBox2.Controls.Add(button_sua_khachhang);
            groupBox2.Controls.Add(button_them_khachhang);
            groupBox2.Font = new Font("Times New Roman", 12F, FontStyle.Regular, GraphicsUnit.Point, 0);
            groupBox2.Location = new Point(751, 52);
            groupBox2.Name = "groupBox2";
            groupBox2.Size = new Size(381, 328);
            groupBox2.TabIndex = 2;
            groupBox2.TabStop = false;
            groupBox2.Text = "Điều Khiển";
            // 
            // button_tatca_khachhang
            // 
            button_tatca_khachhang.BackColor = Color.Tan;
            button_tatca_khachhang.Location = new Point(46, 50);
            button_tatca_khachhang.Name = "button_tatca_khachhang";
            button_tatca_khachhang.Size = new Size(294, 39);
            button_tatca_khachhang.TabIndex = 4;
            button_tatca_khachhang.Text = "Tất Cả";
            button_tatca_khachhang.UseVisualStyleBackColor = false;
            button_tatca_khachhang.Click += button_tatca_khachhang_Click;
            // 
            // button_timkiem_khachhang
            // 
            button_timkiem_khachhang.BackColor = Color.Tan;
            button_timkiem_khachhang.Location = new Point(46, 278);
            button_timkiem_khachhang.Name = "button_timkiem_khachhang";
            button_timkiem_khachhang.Size = new Size(294, 39);
            button_timkiem_khachhang.TabIndex = 3;
            button_timkiem_khachhang.Text = "Tìm Kiếm";
            button_timkiem_khachhang.UseVisualStyleBackColor = false;
            button_timkiem_khachhang.Click += button_timkiem_khachhang_Click;
            // 
            // button_xoa_khachhang
            // 
            button_xoa_khachhang.BackColor = Color.Tan;
            button_xoa_khachhang.Location = new Point(46, 221);
            button_xoa_khachhang.Name = "button_xoa_khachhang";
            button_xoa_khachhang.Size = new Size(294, 39);
            button_xoa_khachhang.TabIndex = 2;
            button_xoa_khachhang.Text = "Xóa";
            button_xoa_khachhang.UseVisualStyleBackColor = false;
            button_xoa_khachhang.Click += button_xoa_khachhang_Click;
            // 
            // button_sua_khachhang
            // 
            button_sua_khachhang.BackColor = Color.Tan;
            button_sua_khachhang.Location = new Point(46, 164);
            button_sua_khachhang.Name = "button_sua_khachhang";
            button_sua_khachhang.Size = new Size(294, 39);
            button_sua_khachhang.TabIndex = 1;
            button_sua_khachhang.Text = "Sửa";
            button_sua_khachhang.UseVisualStyleBackColor = false;
            button_sua_khachhang.Click += button_sua_khachhang_Click;
            // 
            // button_them_khachhang
            // 
            button_them_khachhang.BackColor = Color.Tan;
            button_them_khachhang.Location = new Point(46, 107);
            button_them_khachhang.Name = "button_them_khachhang";
            button_them_khachhang.Size = new Size(294, 39);
            button_them_khachhang.TabIndex = 0;
            button_them_khachhang.Text = "Thêm";
            button_them_khachhang.UseVisualStyleBackColor = false;
            button_them_khachhang.Click += button_them_khachhang_Click;
            // 
            // dataGridView_khachhang
            // 
            dataGridView_khachhang.ColumnHeadersHeightSizeMode = DataGridViewColumnHeadersHeightSizeMode.AutoSize;
            dataGridView_khachhang.Location = new Point(13, 386);
            dataGridView_khachhang.Name = "dataGridView_khachhang";
            dataGridView_khachhang.ReadOnly = true;
            dataGridView_khachhang.RowHeadersWidth = 51;
            dataGridView_khachhang.Size = new Size(1119, 322);
            dataGridView_khachhang.TabIndex = 3;
            dataGridView_khachhang.CellContentClick += dataGridView_khachhang_CellContentClick;
            // 
            // KhachHangControl
            // 
            AutoScaleDimensions = new SizeF(8F, 20F);
            AutoScaleMode = AutoScaleMode.Font;
            BackColor = Color.SkyBlue;
            Controls.Add(dataGridView_khachhang);
            Controls.Add(groupBox2);
            Controls.Add(groupBox1);
            Controls.Add(label1);
            Name = "KhachHangControl";
            Size = new Size(1147, 721);
            Load += KhachHangControl_Load;
            groupBox1.ResumeLayout(false);
            groupBox1.PerformLayout();
            groupBox2.ResumeLayout(false);
            ((System.ComponentModel.ISupportInitialize)dataGridView_khachhang).EndInit();
            ResumeLayout(false);
            PerformLayout();
        }

        #endregion

        private Label label1;
        private GroupBox groupBox1;
        private Label label3;
        private Label label2;
        private Label label9;
        private Label label8;
        private Label label7;
        private Label label6;
        private Label label5;
        private Label label4;
        private TextBox textBox_tenKH_khachhang;
        private TextBox textBox_maKh_khachhang;
        private TextBox textBox_timkiem_khachhang;
        private TextBox textBox_diachi_khachhang;
        private TextBox textBox_email_khachhang;
        private TextBox textBox_sdt_khachhang;
        private RadioButton radioButton_nu_khachhang;
        private RadioButton radioButton_nam_khachhang;
        private DateTimePicker dateTimePicker_ngaysinh_khachhang;
        private GroupBox groupBox2;
        private Button button_timkiem_khachhang;
        private Button button_xoa_khachhang;
        private Button button_sua_khachhang;
        private Button button_them_khachhang;
        private Button button_tatca_khachhang;
        private DataGridView dataGridView_khachhang;
    }
}
